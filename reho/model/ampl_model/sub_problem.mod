...
# Avoid circular flow
#--------------------------------------------------------------------------------------------------------------------#
var grid_flow_direction{h in HousesOfLayer['Electricity'], p in Period, t in Time[p]} binary; # 1 if supply-on, 0 if demand-on
subject to Supply_on_when_demand_off{l in ResourceBalances, h in HousesOfLayer[l], p in Period, t in Time[p]}:
	Grid_supply['Electricity',h,p,t] <= Line_ext[h, 'Electricity'] * grid_flow_direction[h,p,t];

subject to Supply_off_when_demand_on{h in HousesOfLayer['Electricity'], p in Period, t in Time[p]}:
   Grid_demand['Electricity',h,p,t] <= Line_ext[h,'Electricity'] * (1 - grid_flow_direction[h,p,t]);
