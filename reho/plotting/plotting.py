...
def plot_expenses(results, plot='costs', indexed_on='Scn_ID', label='EN_long', add_annotation=True, per_m2=False, include_avoided=False, additional_costs={},
                  additional_gwp={},
                  scc=0.177,
...
def plot_profiles(df_Results, units_to_plot, style='plotly', label='EN_long', color='ColorPastel', resolution='weekly', plot_curtailment=False,
    ...
    ----------
    df_Results: pd.DataFrame
        Coming from REHO results (already extracted from the desired *Scn_ID* or *Pareto_ID*).
    units_to_plot: list
        Units to be plotted.
    style: str
...
def plot_storage_profile(df_Results, resolution='daily', storage_ID="all"):
    ...
    counter = 1
    if storage_ID == 'all':
        df_interperiod = df_Results["df_Interperiod"]
        list_stor = list(df_interperiod.keys())
        if len(list_stor) > 0:
        ...
        if type(storage_ID) is list:
            ...
            list_stor = [storage_ID]
        df_interperiod = df_Results["df_Interperiod"].loc[:, (df_Results["df_Interperiod"] != 0).any(axis=0)]
        if len(list_stor) > 0:
            storage_SOC_tot = df_interperiod.groupby(level=1).sum()
            fig = make_subplots(rows=len(list_stor), cols=1, shared_xaxes=True, vertical_spacing=0.02)
            for storage in list_stor:
