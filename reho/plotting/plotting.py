...
## I've added the possibility of adding the year, transformer id and the dictionnary of parameters in the function's arguments. It is also now possible to express the results per capita
## and plot multiple results on a same plot, like a Pareto. The replacement costs have been approached in this version of REHO by doubling the cost of the mobility vehicles' investment costs.
def plot_performance(results, plot='costs', indexed_on='Scn_ID', label='EN_long', add_annotation=True, per_m2=False, additional_costs=None, additional_gwp=None,
                     scc=0.177, title=None, filename=None, export_format='png', scaling_factor=1, return_df=False, per_pers=False, parameters=None, year=None, transformer=None, single=True):
    """
    ...
    mobility_modes = ['TrolleyBus_district','ElectricBus_district','DieselBus_district','Metro_district']
    if plot == 'costs':
        ...
        if single:
            for x in mobility_modes:
                df_costs.loc[('totex',0,'Network')]['investment'][x] *= 2           # attempt to account for replacement costs rather than merging the code that fixes it

            if per_pers:
                df_costs.loc[('totex',0,'Network'),'operation']['costs_Electricity'] = 0.3625 * results['totex'][0]['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                population = parameters[year][transformer]['Population'] 
                #print(population)                                      
                df_costs = df_costs / population
                change_data.loc['y_axis']['FR'] = "Coûts [CHF/pers/an]"
                change_data.loc['y_axis']['EN'] = "Costs [CHF/cap/y]"
                decimal = 0

        else: 
            for x in mobility_modes:
                df_costs.loc[('pareto','baseline-24','Network')]['investment'][x] *= 2           # attempt to account for replacement costs rather than merging the code that fixes it
                df_costs.loc[('pareto','PV/HP-24','Network')]['investment'][x] *= 2
                df_costs.loc[('pareto','baseline-30','Network')]['investment'][x] *= 2           
                df_costs.loc[('pareto','PV/HP-30','Network')]['investment'][x] *= 2
                df_costs.loc[('pareto','baseline-50','Network')]['investment'][x] *= 2           
                df_costs.loc[('pareto','PV/HP-50','Network')]['investment'][x] *= 2

            if per_pers:
                df_costs.loc[('pareto','baseline-24','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-24']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                df_costs.loc[('pareto','PV/HP-24','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-24']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                df_costs.loc[('pareto','baseline-30','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-30']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                df_costs.loc[('pareto','PV/HP-30','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-30']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000
                df_costs.loc[('pareto','baseline-50','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-50']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                df_costs.loc[('pareto','PV/HP-50','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-50']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000
                population24 = parameters[2024][transformer]['Population']
                population30 = parameters[2030][transformer]['Population']
                population50 = parameters[2050][transformer]['Population'] 
                #print(population)                                      
                #df_costs = df_costs / population
                df_costs.loc[('pareto','baseline-24','Network')] = df_costs.loc[('pareto','baseline-24','Network')] / population24
                df_costs.loc[('pareto','PV/HP-24','Network')] = df_costs.loc[('pareto','PV/HP-24','Network')] / population24
                df_costs.loc[('pareto','baseline-30','Network')] = df_costs.loc[('pareto','baseline-30','Network')] / population30
                df_costs.loc[('pareto','PV/HP-30','Network')] = df_costs.loc[('pareto','PV/HP-30','Network')] / population30
                df_costs.loc[('pareto','baseline-50','Network')] = df_costs.loc[('pareto','baseline-50','Network')] / population50
                df_costs.loc[('pareto','PV/HP-50','Network')] = df_costs.loc[('pareto','PV/HP-50','Network')] / population50
        ...
        if single:
            for x in mobility_modes:
                df_impact.loc[('totex',0,'Network')]['investment'][x] *= 2           # attempt to account for replacement costs rather than merging the code that fixes it

            if per_pers:
                df_impact.loc[('totex',0,'Network'),'operation']['costs_Electricity'] = 0.3625 * results['totex'][0]['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                population = parameters[year][transformer]['Population'] 
                #print(population)                                      
                df_impact = df_impact / population
                change_data.loc['y_axis']['FR'] = "Émissions [kgCO2/pers/an]"
                change_data.loc['y_axis']['EN'] = "Emissions [kgCO2/cap/y]"
                decimal = 0

        else: 
            for x in mobility_modes:
                df_impact.loc[('pareto','baseline-24','Network')]['investment'][x] *= 2           # attempt to account for replacement costs rather than merging the code that fixes it
                df_impact.loc[('pareto','PV/HP-24','Network')]['investment'][x] *= 2
                df_impact.loc[('pareto','baseline-30','Network')]['investment'][x] *= 2           
                df_impact.loc[('pareto','PV/HP-30','Network')]['investment'][x] *= 2
                df_impact.loc[('pareto','baseline-50','Network')]['investment'][x] *= 2           
                df_impact.loc[('pareto','PV/HP-50','Network')]['investment'][x] *= 2

            if per_pers:
                df_impact.loc[('pareto','baseline-24','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-24']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          # the price comes from https://www.prix-electricite.elcom.admin.ch/municipality/5586?period=2024
                df_impact.loc[('pareto','PV/HP-24','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-24']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          
                df_impact.loc[('pareto','baseline-30','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-30']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          
                df_impact.loc[('pareto','PV/HP-30','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-30']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000
                df_impact.loc[('pareto','baseline-50','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['baseline-50']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000          
                df_impact.loc[('pareto','PV/HP-50','Network'),'operation']['costs_Electricity'] = 0.3625 * results['pareto']['PV/HP-50']['df_Annuals'].loc[('Electricity', 'Network'), 'Supply_MWh'] * 1000
                #population = parameters[year][transformer]['Population'] 
                #print(population)                                      
                #df_impact = df_impact / population
                population24 = parameters[2024][transformer]['Population']
                ...
                #print(population)                                      
                #df_costs = df_costs / population
                df_impact.loc[('pareto','baseline-24','Network')] = df_impact.loc[('pareto','baseline-24','Network')] / population24
                df_impact.loc[('pareto','PV/HP-24','Network')] = df_impact.loc[('pareto','PV/HP-24','Network')] / population24
                df_impact.loc[('pareto','baseline-30','Network')] = df_impact.loc[('pareto','baseline-30','Network')] / population30
                df_impact.loc[('pareto','PV/HP-30','Network')] = df_impact.loc[('pareto','PV/HP-30','Network')] / population30
                df_impact.loc[('pareto','baseline-50','Network')] = df_impact.loc[('pareto','baseline-50','Network')] / population50
                df_impact.loc[('pareto','PV/HP-50','Network')] = df_impact.loc[('pareto','PV/HP-50','Network')] / population50
    ...
    if filename is not None:
        ...
        if export_format == 'png' or export_format == 'pdf':
            print('png')
            fig.write_image('.figure/' + filename + '.' + export_format)
