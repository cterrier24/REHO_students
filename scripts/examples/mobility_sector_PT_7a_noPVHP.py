######################################################################################################################################
#### Run the 'baseline' scenarios with no PV and HP of each typical district.                                                     ####
#### Results are available under the xlsx and pkl format. To plot the Sankeys and cost/gwp plots, use 7a_plotting.py              ####
#### !!! DO NOT comment the lines in master_problem.py rabout the PT.mod file (mobility is part of the scenario indeed)           ####
######################################################################################################################################
...
dict_param_3230_2050 = { "Population": 7700,               # 190 buildings             
                    "DailyDist" : {"long" : 20,
                                   'short' : 10},
                    "n_rames": 10,
                    "n_trolley": 87,
...
if __name__ == '__main__':
    ...
    for year in [2050]:
        ...
        for transformer in [3230]:
            ...
            scenario['EMOO'] = {}
            scenario['exclude_units'] = ['NG_Cogeneration','ThermalSolar','PV','HeatPump']
            ...
            # Save results
            reho.save_results(format=['pickle'], filename=f'7a_{transformer}_{year}_no_PVHP_fix', erase_file=True)

            # Plot results
            #plotting.plot_sankey(reho.results['totex'][0], label='EN_long', color='ColorPastel', title=f"Sankey diagram {transformer}_{year}").show()
