...
if __name__ == '__main__':
    ...
    scenario['exclude_units'] = ['PV', 'Battery', 'HeatPump','ThermalSolar', 'Battery_district', 'ElectricBike_district']
    ...
    grids["NaturalGas"]["ReinforcementOfNetwork"] = np.array([4000])
    ...
    Network_ext = pd.DataFrame([100, 4000, 2000, 2000], index=["Electricity", "NaturalGas", "Gasoline", "Mobility"],
                               columns=["Network_ext"])

    era = np.sum([qbuildings_data["buildings_data"][b]['ERA'] for b in qbuildings_data["buildings_data"]])

    modal_split = pd.DataFrame({"min_short": [0, 0, 0.4, 0, 0], "max_short": [0.1, 0.3, 0.7, 0, 0]},
                               index=['MD', 'PT', 'cars', 'EV_district', 'ElectricBike_district'])

    parameters = {'Network_ext': Network_ext, "DailyDist": {'short': 25}, "Population": era/46}#, "ff_EV": 1.56}
    set_indexed = {"Distances": ["short"]}

    units = infrastructure.initialize_units(scenario, grids, district_data=True)

    # Define maximum rent affordable (optional)
    reho = ActorsProblem(qbuildings_data=qbuildings_data, units=units, parameters=parameters, grids=grids, cluster=cluster, scenario=scenario, method=method, DW_params={'max_iter': 4}, solver="gurobiasl")
    ...
    # Save results
    reho.save_results(format=["pickle"], filename='8a_without_El')