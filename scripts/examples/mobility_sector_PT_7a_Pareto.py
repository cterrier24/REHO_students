...
dict_param_3217_2030 = { "Population": 8300,               # 107 buildings, 8300 personnes     
                    "DailyDist" : {"long" : 20,
                                   'short' : 10},
                    "n_rames": 24,
                    "n_trolley": 28,
...
if __name__ == '__main__':
    ...
    for year in [2024]:
        ...
        for transformer in [3230]:
            ...
            # Save results
            reho.save_results(format=['xlsx', 'pickle'], filename=f'7a_{transformer}_{year}_Pareto', erase_file=True)
            ...
            plotting.plot_pareto(reho.results, title=f'Pareto_{transformer}_{year}').show()

            # Sankey diagram
            for key in reho.results['pareto'].keys():
                plotting.plot_sankey(reho.results['pareto'][key], label='EN_long', color='ColorPastel', title=f"Sankey diagram {transformer}-{year}").show()
